...
async def test_jsonb_roundtrip_and_containment(pg_session) -> None:
    ...
    """
    await pg_session.execute(
        text("CREATE TEMP TABLE _t_jsonb (id uuid PRIMARY KEY, data jsonb) ON COMMIT DROP")
    )
    ...
    got = (
        await pg_session.execute(
            text("SELECT data FROM _t_jsonb WHERE id = CAST(:id AS uuid)"), {"id": str(row_id)}
        )
    ).scalar_one()
...
async def test_ilike_is_case_insensitive_unlike_like(pg_session) -> None:
    ...
    """
    await pg_session.execute(
        text("CREATE TEMP TABLE _t_search (id uuid PRIMARY KEY, name text) ON COMMIT DROP")
    )
    await pg_session.execute(
    ...
    ilike_hits = (
        await pg_session.execute(
            text("SELECT count(*) FROM _t_search WHERE name ILIKE :q"), {"q": "stahlbeton%"}
        )
    ).scalar_one()
    like_hits = (
        await pg_session.execute(
            text("SELECT count(*) FROM _t_search WHERE name LIKE :q"), {"q": "stahlbeton%"}
        )
    ).scalar_one()
...
async def test_uuid_primary_key_roundtrip(pg_session) -> None:
    ...
    await pg_session.execute(
        text("CREATE TEMP TABLE _t_uuid (id uuid PRIMARY KEY) ON COMMIT DROP")
    )
    rid = uuid.uuid4()
    await pg_session.execute(
        text("INSERT INTO _t_uuid (id) VALUES (CAST(:id AS uuid))"), {"id": str(rid)}
    )
