...
We use a real PostgreSQL DB (the transaction-isolated ``oe_test_unit``
database from ``tests._pg``) to exercise the JSON path expression -
the dialect-aware ``_classification_expr`` is the riskiest part of the
patch, so 20 fixture rows on the real engine beats a mock-heavy test for
...
async def session() -> AsyncSession:
    """Transaction-isolated PostgreSQL session seeded with 20 cost items."""
