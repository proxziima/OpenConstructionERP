...
3. Build the full schema via ``create_all`` and assert every declared
   composite index actually lands in the PostgreSQL catalog.
...
# ── Live schema introspection (PostgreSQL) ─────────────────────────────────


def test_indexes_land_in_schema_after_create_all() -> None:
    """After ``Base.metadata.create_all``, the composite indexes live in the
    PostgreSQL catalog.

    The model-level ``Index(...)`` declarations are what ``create_all`` reads -
    we don't need to run the alembic migration to assert presence (fresh
    install paths use ``create_all``, not alembic, on bootstrap). The alembic
    migration matters for upgrade paths on populated prod databases.

    The schema-loaded unit database is built once per session via the same
    ``create_all`` the bootstrap uses, so inspecting it exercises that path.
    """
    ...
    try:
        ...
        # Each composite from the migration is reachable via the model.
        expected_per_table = {
            "oe_boq_position": {
...
def test_legacy_single_column_fk_indexes_still_present() -> None:
    ...
    pattern still benefits from the single-column index, so the planner
    can choose between them per query.
    """
    from sqlalchemy import inspect
